...
const Haushaltsbuch = {
  ...
  eintrag_entfernen(timestamp) {
      ...
      for (let i = 0; i < this.eintraege.length; i++) {
        if(parseInt(timestamp) === this.eintraege[i].get("timestamp")){
          start_index = i;
          break;
        }
  ...
  eintraege_anzeigen() {
    ...
    this.eintraege.forEach((eintrag) => eintragsliste.insertAdjacentElement("beforeend",
  ...
  html_gesamtbilanz_generieren() {
    ...
    let bilanz_betrag = document.createElement("span");
      this.gesamt_bilanz.get("gesamt") >= 0
        ? bilanz_betrag.setAttribute("class", "positiv")
        : bilanz_betrag.setAttribute("class", "negativ");
 
    bilanz_betrag.textContent = `${(this.gesamt_bilanz.get("gesamt") / 100)
      .toFixed(2)
      .replace(/\./, ",")} €`;
