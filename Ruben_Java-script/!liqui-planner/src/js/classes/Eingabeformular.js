...
constructor(){
    ...
    _formulardaten_validieren(formulardaten){
        ...
        if(formulardaten.titel ===""){
            formular_fehler.push("Titel!")
        }
        if(formulardaten.datum === null){
            formular_fehler.push("Datum");
        }
        if(isNaN(formulardaten.betrag)){
            formular_fehler.push("Betrag!")
        }
        return formular_fehler;
    }

  _absenden_event_hinzufuegen(eingabeformular) {
    eingabeformular.querySelector("#eingabeformular").addEventListener("submit", e => {e.preventDefault();
        ...
        if( formular_fehler.length === 0 ){
            haushalts_buch.eintrag_hinzufuegen(formulardaten);
            ...
            let fehler = new Fehler(
              "Folgende Felder wurden nicht korrekt ausgefüllt: ", formular_fehler);
            fehler.anzeigen(); 
        };            
      });
  }
